<!--
 Created by Xuz on 2019/03/26.
 xuz[at]live[dot]cn
 编辑系统模块
-->
<template>
    <a-modal title="编辑系统模块"
             :width="1024"
             :visible="visible"
             :maskClosable="false"
             @ok="onSubmit"
                ...
                <a-col :span="12">
                    <a-form-item label='文档类别'
                        ...
                        <a-select :options="cateList" v-decorator="[
                                    'caseTypeId',
                                    {initialValue:modalData.cate_id}
                                ]" allowClear />
                    </a-form-item>
                </a-col>
                <a-col :span="12">
                    <a-form-item label='模块名称'
                        ...
                        <a-input v-decorator="[
                                    'name',
                                    {initialValue:modalData.name, rules: [{ required: true, message: '请填写模块名称' }]}
                                ]" placeholder='模块名称'/>
                    </a-form-item>
                </a-col>
                <a-col :span="12">
                    <a-form-item
                        ...
                        <a-input-number v-decorator="[
                                    'sort',
                                    {initialValue:modalData.sort, rules: [{ required: true, type: 'number', message: '请填写数字类型顺序号' }]}
                                ]" placeholder='顺序号'/>
                    </a-form-item>
                </a-col>
                <a-col :span="12">
...
export default {
  ...
  methods: {
    ...
    preModuleEdit (module) {
      ...
      //
      that.$http.get('/api/1/module/get_module.json?id=' + module.id).then(function (resp) {
        that.modalData = resp.result
        //
        const result = [...resp.result.extras.permissions.map(item => item.id)]
      ...
      that.$http.get('/api/1/module/get_modules.json').then(function (resp) {
        let result = []
        that.mapTreeSelectData(result, resp.result, [module.id])
    ...
    onSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        //
        data.id = that.modalData.id
        data.parentId = that.modalData.parent_id
        data.permIds = perms.join(',')
        //
        that.modalLoading = true
        that.$http.post('/api/1/module/edit_module.json', qs.stringify(data)).then(function (resp) {
          that.modalLoading = false
          that.$message.success('编辑模块成功')
          that.$emit('success')
        })
      })
    },
