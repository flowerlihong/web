    ...
    <a-modal title="添加意见类型别名"
        ...
        <a-form :form="form">
            ...
            <a-form-item
                ...
                <a-input v-decorator="[
                            'aliasName',
                            {rules: [{ required: true, message: '请填写意见类型别名' }]}]" placeholder='意见类型别名'/>
            </a-form-item>
            <a-form-item
                    label='所属意见类型'
                    :labelCol="{ span: 5 }"
                ...
                <a-select :options="opinionTypeList" v-decorator="[
                            'opinionTypeId',
                            {rules: [{ required: true, message: '请选择所属意见类型' }]}]" placeholder='所属意见类型'/>
            </a-form-item>
            <a-form-item
                    label='顺序号'
                    :labelCol="{ span: 5 }"
  ...
  methods: {
    ...
    onSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        this.modalLoading = true
        this.$http.post('/api/1/workflow/cate/add_opinion_alias.json', qs.stringify(params)).then(function () {
