        ...
        <a-card>
            <a-form :form="queryTripForm">
                <table class="fx-form">
                    <tr>
                        ...
                        <th>排序</th>
                        <td>
                            <a-select  style='width: 120px' v-model="queryForm.sort">
                                <a-select-option value="">不限</a-select-option>
                                <a-select-option value="1">申请时间</a-select-option>
                                <a-select-option value="2">外出时间</a-select-option>
                                <a-select-option value="3">返回时间</a-select-option>
                            </a-select>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <th>外出类型</th>
                        <td><dict-cascader dictKey="BUSINESSTRIP.TYPES" style="width:174px" v-model="queryForm.types"/></td>
                    </tr>
                    <tr>
                        <th>计划返回时间</th>
                        <td>
            ...
            <s-table
                ...
                <template slot="userName" slot-scope=" record">
                    <!--<a @click="onEditPlan(record)">{{record.userName}}</a>-->
                ...
                <template slot="address" slot-scope=" record">
                    <div v-if="record.address.length > 10">
                        <a @click="onView(record)">{{record.address.substring(0,19)}}...</a>
                    </div>
                    <div v-else>
  ...
  methods: {
    ...
    onSearch1 () {
      this.queryTripForm.validateFields((err, values) => {
        ...
        const that = this
        let data = that.queryTripForm.getFieldsValue()
        console.log(data)
      })
    },
    onSearch () {
      ...
      if (this.queryForm.createdAtEnd) {
        this.queryParam.createdAtEnd = this.queryParam.createdAtEnd.format('YYYY-MM-DD') + ' 23:59:59'
      }
      // 计划外出
      ...
      if (this.queryParam.planStartAtEnd) {
        this.queryParam.planStartAtEnd = this.queryParam.planStartAtEnd.format('YYYY-MM-DD') + ' 23:59:59'
      }
      // 计划返回
      ...
      if (this.queryParam.planEndAtEnd) {
        this.queryParam.planEndAtEnd = this.queryParam.planEndAtEnd.format('YYYY-MM-DD') + ' 23:59:59'
      }
