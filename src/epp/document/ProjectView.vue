<template>
    <a-card>
        <div class="co-body-top">
            ...
            <router-link to="">
                <a-button type="primary" icon="plus" :size="size" @click="onAdd">新增解决方案</a-button>
            </router-link>
        </div>
        <a-list itemLayout="horizontal" :dataSource="projectList">
        ...
        <project-add ref="projectAdd" :visible="modalVisible.projectAdd" @cancel="onAddCancel"
                 @success="onAddSucess" :companyId="companyId"></project-add>
        <project-edit ref="projectEdit" :visible="modalVisible.projectEdit" @cancel="onEditCancel"
                 @success="onEditSucess"></project-edit>
    </a-card>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    init () {
      const that = this
      this.$http.get('/api/1/project/get_project_list.json?companyId=' + that.companyId).then(function (res) {
        if (res.code !== 200) {
          return
        }
    ...
    year () {
      const that = this
      this.$http.get('/api/1/project/get_year_list.json?companyId=' + that.companyId).then(function (res) {
        if (res.code !== 200) {
          return
        }
    ...
    onAdd () {
      this.modalVisible.projectAdd = true
    },
    onAddCancel () {
      this.modalVisible.projectAdd = false
    },
    onAddSucess () {
      this.modalVisible.projectAdd = false
      this.init()
    ...
    onEdit (id) {
      this.modalVisible.projectEdit = true
      this.$refs.projectEdit.onEdit(id)
    },
    onEditCancel () {
      this.modalVisible.projectEdit = false
    },
    onEditSucess () {
      this.modalVisible.projectEdit = false
      this.init()
    ...
    handleChange (value) {
      ...
      this.$http.post('/api/1/project/get_project_list_by_year.json', qs.stringify({ companyId: that.companyId, year: value })).then(function (res) {
        if (res.code !== 200) {
          return
        }
        let result = res.result
    ...
    onSearch (value) {
      const that = this
      this.$http.post('/api/1/project/search_project_by_key.json', qs.stringify({ companyId: that.companyId, year: that.selectYear, title: value })).then(function (res) {
        if (res.code !== 200) {
          return
        }
        let result = res.result
