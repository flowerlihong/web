  ...
  methods: {
    ...
    uploadImgRemote () {
      ...
      that.$refs.cropper.getCropData(data => {
        that.$http.post('/api/1/staff/certificate_avatar_base64.json?id=' + that.certificateId, qs.stringify({ avatar: data, staffCertificate: that.staffCertificate })).then(function (resp) {
          that.$message.success('上传证书图片成功')
          that.confirmLoading = false
          that.$emit('change', resp.msg, that.staffCertificate)
          that.$emit('success')
        })
      })
    },
