...
export default {
  ...
  methods: {
    moment,
    onEdit (id) {
      this.caseModal.id = id
      this.init()
    },
    init () {
      const that = this
      this.$http.get('/api/1/case/get_case_by_id.json?id=' + that.caseModal.id).then(function (res) {
    ...
    onSubmit () {
      this.form.validateFields((err, values) => {
        ...
        if (data.id === '') {
          ...
          this.loading = true
          this.$http.post('/api/1/case/edit_case.json', qs.stringify(data)).then(function (res) {
            if (res.code !== 200) {
              return
            }
            that.$message.success('合同案例修改成功')
