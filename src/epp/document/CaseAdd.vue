 ...
 <a-modal title="新增合同案例"
      ...
      <a-form :form="form">
        <a-form-item label="合同名称" v-bind="formItemLayout">
            <a-input placeholder="请输入合同标题"
                      v-decorator="['title',{ rules: [{ required: true, message: '请输入合同标题' }, {max: 200, message: '标题不能超过200个字'}]}]"></a-input>
        </a-form-item>
        <a-form-item label="文档类型" v-bind="formItemLayout">
            <a-row :gutter="6">
              <a-col :xl="6" :lg="24" :md="24" :sm="24" :xs="24">
                <a-select
                  placeholder="请选择"
                  v-decorator="['type',{ rules: [{ required: true, message: '请选择文档类型'}]}]">
        ...
        <a-form-item label="文档页数" v-bind="formItemLayout">
            <a-input-number placeholder="请输入页数"
                      v-decorator="['page',{ rules: [{ required: true, message: '请输入文档页数' }]}]"></a-input-number>
        </a-form-item>
        ...
        <a-form-item label="上传文件" >
          <upload-selector :referId='caseModal.id' configKey='DOCUMENT.FILE' />
        </a-form-item>
        <a-form-item label="标签" v-bind="formItemLayout">
            <a-input placeholder="请输入标签，多个标签请以','间隔"
                      v-decorator="['tags',{ rules: [{ required: true, message: '请输入标签' }]}]"></a-input>
        </a-form-item>
    ...
    onSubmit () {
      this.form.validateFields((err, values) => {
        ...
        if (data.companyId === '') {
          ...
          this.loading = true
          this.$http.post('/api/1/case/add_case.json', qs.stringify(data)).then(function (res) {
            if (res.code !== 200) {
              return
            }
            that.$message.success('合同案例添加成功')
