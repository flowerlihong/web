      ...
      <a-form :form="form">
        <a-form-item label="方案名称" v-bind="formItemLayout">
            <a-input placeholder="请输入文档标题"
                      v-decorator="['title',{ rules: [{ required: true, message: '请输入文档标题' }, {max: 200, message: '标题不能超过200个字'}]}]"></a-input>
        </a-form-item>
    ...
    onSubmit () {
      this.form.validateFields((err, values) => {
        ...
        if (data.companyId === '') {
          ...
          this.loading = true
          this.$http.post('/api/1/project/add_project.json', qs.stringify(data)).then(function (res) {
            if (res.code !== 200) {
              return
            }
            that.$message.success('解决方案添加成功')
