            ...
            <a-form :form="form">
                ...
                <table class="fx-form">
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入收款单位地址" v-decorator="[
                                    'receiveUnitAddress',
                                    { rules: [{ required: true, message: '请输入收款单位地址' }]}
                                ]"/>
                            </a-form-item>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入收款单位开户银行及支行名称" v-decorator="[
                                    'accountName',
                                    { rules: [{ required: true, message: '请输入收款单位开户银行及支行名称' }]}
                                ]"/>
                            </a-form-item>
                        </td>
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入收款账号" v-decorator="[
                                    'account',
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入收款单位省份" v-decorator="[
                                    'province',
                                    { rules: [{ required: true, message: '请输入收款单位省份' }]}
                                ]"/>
                            </a-form-item>
                        </td>
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入收款单位地市" v-decorator="[
                                    'city',
                                    { rules: [{ required: true, message: '请输入收款单位地市' }]}
                                ]"/>
                            </a-form-item>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <a-form-item>
                                <a-input placeholder="请输入用途" v-decorator="[
                                    'purpose',
                        ...
                        <td>
                            <a-form-item>
                                <a-input-number :min="0" v-decorator="[
                                    'payMoney',
                                    {initialValue: 0,rules: [{ required: true, message: '请输入已付金额' }]}
                                ]"/>
                            </a-form-item>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <a-form-item>
                                <dict-radio dictKey="PAY.WAY" @change="onRadioChange" v-decorator="['payWay',{initialValue:'2', rules: [{ required: true, message: '请选择付款方式' }]}]"/>
                        ...
                        <td>
                            <a-form-item>
                                <user-selector v-decorator="[
                                    'userId',
                                    {initialValue: submitForm.userId, rules: [{ required: true, message: '请选择申请人' }]}
                                ]" />
                            </a-form-item>
                        </td>
                    </tr>
                    <tr>
                        <th class="not-null">备注</th>
                        <td>
                            <a-form-item>
                            ...
                            <a-form-item>
                                <a-radio-group name="radioGroup"
                                   v-decorator="['hasCheck',
                                    {initialValue: '1',rules: [{ required: true, message: '请选择是否收到支票' }]}
                                ]">
    ...
    submit () {
      this.form.validateFields((err, values) => {
        ...
        let result = this.payApplySubjects.every(function (item) {
          if (item.subject === '' || item.price === '' || item.price === 0) {
            return false
          }
          return true
        })
        ...
        data.caseTypeId = this.submitForm.caseTypeId
        // data.createdAt = data.createdAt.format('YYYY-MM-DD')
        data.payApplySubjects = this.payApplySubjects
        const that = this
        that.loading = true
        that.$http.post('/api/1/pay_apply/add_pay_apply.json', qs.stringify(data, { allowDots: true })).then(function (resp) {
          that.loading = false
          if (resp.code !== 200) {
            return
          }
          that.$message.success('提交成功')
          that.$router.push({ path: '/pay/apply/view?id=' + resp.caseId })
        })
      })
    }
  }
