            ...
            <div>
                <div class="fx-select-dropdown" :style="style">
                    <div class="fx-select-panel" style="width:300px;">
                        <a-tree checkStrictly :selectedKeys="treeSelectedKeys" :checkable="multiple" showIcon v-model="treeCheckedKeys" :expandedKeys="treeExpandedKeys" @expand="onTreeExpand" :autoExpandParent="autoExpandParent" :treeData="treeData" :loadData="onLoadData" @check="onTreeCheck" @select="onTreeSelect">
  ...
  methods: {
    ...
    onLoadData (treeNode) {
      return new Promise((resolve) => {
        ...
        const that = this
        that.$http.get('/api/1/comp/org_node.json?id=' + treeNode.dataRef.key).then(function (response) {
    ...
    onOrgFetch () {
      const that = this
      that.$http.get('/api/1/comp/org_node.json').then(function (response) {
    ...
    onDefaultValueFetch () {
      ...
      const that = this
      that.$http.get('/api/1/org/get_orgs.json?ids=' + this.value).then(function (response) {
