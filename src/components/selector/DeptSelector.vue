...
export default {
  ...
  methods: {
    ...
    onTreeCheck (checkedKeys, info) {
      let tmp = []
      checkedKeys.forEach(id => tmp.push(this.flatData.get(id)))
      this.selectedData = tmp
      this.selectedIds = [...checkedKeys]
    ...
    onLoadData (treeNode) {
      return new Promise((resolve) => {
        ...
        const that = this
        that.$http.get('/api/1/comp/org_dept_node.json?id=' + treeNode.dataRef.key).then(function (response) {
    ...
    onOrgFetch () {
      const that = this
      that.$http.get('/api/1/comp/org_dept_node.json').then(function (response) {
    ...
    onDefaultDeptFetch () {
      ...
      const that = this
      that.$http.get('/api/1/org/get_depts.json?ids=' + this.value).then(function (response) {
