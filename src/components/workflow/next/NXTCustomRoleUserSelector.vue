                ...
                <div class="fx-select-panel" style="width:120px">
                    ...
                    <template v-else>
                        <a-radio-group @change="onRadioUserChange" :value="selectedIds.length > 0 ? selectedIds[0] : ''">
                            <div v-for="role in roleData" :key="role.id">
                                <div class="role-name">{{role.name}}</div>
                                <a-radio v-for="user in role.extras.users" :key="'c' + user.id" :value="user.id">{{ user.name }}</a-radio>
                            </div>
                        </a-radio-group>
                    </template>
                </div>
  ...
  methods: {
    update (wfType, routeData) {
      ...
      if (this.bGroupSendMode) {
        ...
        this.roleData = routeData.roles
        this.userData = this.mapUserData(routeData.roles)
      }
    },
    getType () {
      return this.wfType
    ...
    mapUserData (roles) {
      ...
      roles.forEach(function (role) {
        let tmp = role.extras.users.map(item => {
          return {
            id: item.id,
            name: item.name
          }
