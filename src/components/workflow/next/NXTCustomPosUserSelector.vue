  ...
  methods: {
    update (wfType, routeData) {
      ...
      if (this.bOrder) {
        ...
        console.log('TODO show order message')
      }

      if (this.bGroupSendMode) {
        this.userData = this.mapGroupUserData(routeData.poses)
      } else {
        this.userData = this.mapUserData(routeData.poses)
      }
    },
    getType () {
      return this.wfType
    ...
    mapUserData (poses) {
      ...
      poses.forEach(function (pos) {
        let tmp = pos.extras.users.map(item => {
          return {
            id: item.id,
            name: item.name
          }
    ...
    mapGroupUserData (poses) {
      let users = []
      poses.forEach(function (pos) {
        users.push({ id: pos.id, name: pos.deptName + '-' + pos.name })
      })
      return users
    }
  }
