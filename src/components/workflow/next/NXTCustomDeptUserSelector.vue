  ...
  methods: {
    update (wfType, routeData) {
      ...
      if (this.bGroupSendMode) {
        this.userData = this.mapGroupUserData(routeData.depts)
      } else {
        this.userData = this.mapUserData(routeData.depts)
      }
    },
    getType () {
      return this.wfType
    ...
    mapGroupUserData (depts) {
      let users = []
      depts.forEach(function (dept) {
        users.push({ id: dept.id, name: dept.orgName + '-' + dept.name })
      })
      return users
    }
  }
